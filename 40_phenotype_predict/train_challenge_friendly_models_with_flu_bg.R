...
ced_tet_samples <- all_samples %>%
  filter(biosource == 'PBMC' & disease == 'Ced' & disease_status == 'UCD') %>%
  filter(sample_category != 'full' & (is.na(note) | note != 'exclude_ac'))

all_samples %>% 
  filter(unique_name == '22-002-01-12 Flu+ All CD4.fcs_full') %>% 
  bind_rows(ced_tet_samples) %>% 
  identity() ->
  train_samples

full_train_df <- train_samples %>%
  mutate(h5path = glue('samples/{unique_name}')) %>%
  .$h5path %>%
  map(partial(read_dataset, 'out/cytof.h5')) %>%
  map(~ as_tibble(.x[, predmarkers$fcs_name])) %>% 
  set_names(train_samples$unique_name) %>% 
...
SEED <- 1234
set.seed(SEED)
control <- trainControl(method="repeatedcv", number=10, repeats=4, classProbs= TRUE, summaryFunction = multiClassSummary, search = 'grid')
metric <- "logLoss"
...
train_df <- full_train_df %>% 
  ...
  select(-unique_name, -disease_status, -CD39, -CD62L, -CCR7)
nparam <- ncol(train_df) - 1
mtry <- sqrt(nparam)
mtry_seek_range <- seq(sqrt(nparam / 1.7), sqrt(nparam*1.2), length.out = 4)
tunegrid <- expand.grid(.mtry = mtry_seek_range)
...
toc()
print(m_rf)
saveRDS(m_rf, file = 'out/predict_tetp/model_rf_tetp_ucd_no_CD39_CD62L_CCR7_with_cancerflu.robj')
