...
##
## Train GFD UCD with CD62L and CCR7
##
train_df <- full_train_df %>% 
  select(-unique_name, -disease_status)
nparam <- ncol(train_df) - 1
mtry <- sqrt(nparam)
tunegrid <- expand.grid(.mtry = c(1:floor(mtry), mtry, ceiling(mtry):(nparam / 3)))
...
toc()
print(m_rf)
saveRDS(m_rf, file = 'out/predict_tetp/model_rf_tetp_gfd_ucd.robj')
m_rf_gfd_ucd <- m_rf
...
toc()
print(m_rf)
saveRDS(m_rf, file = 'out/predict_tetp/model_rf_tetp_ucd.robj')
...
##
## Train UCD without CD62L and CCR7
##
train_df <- full_train_df %>% 
  filter(disease_status == 'UCD') %>% 
  select(-unique_name, -disease_status, -CD62L, -CCR7)
nparam <- ncol(train_df) - 1
mtry <- sqrt(nparam)
...
toc()
print(m_rf)
saveRDS(m_rf, file = 'out/predict_tetp/model_rf_tetp_ucd_no_CD62L_or_CCR7.robj')
