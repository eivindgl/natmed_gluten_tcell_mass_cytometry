###
### Reads all FCS files and stores ashinh results in out/cytof.h5
### The h5 file is subsequently reads by other scripts to read input data.
...
read_all_fcs_files <- function(sample_meta, only_functional_AB = TRUE, 
                               fcs_trans = list(func = asinh, cofactor = 5)) {
  cytof_panels <- read_csv('meta/cytof_panels.csv') %>% 
  ...
  cytof_panels %>% 
    ...
    identity() ->
    metals_that_varies
  fcsAB_to_AB <- read_csv('meta/fcs_AB_to_Antibody.csv')
  
  ab_meta <- read_csv('meta/antibody_meta.csv')
...
samples_meta <- all_samples %>% 
  mutate(path = file.path('fcs', filename))
all_fcs <- read_all_fcs_files(sample_meta = samples_meta, fcs_trans = list(func = asinh, cofactor = 1))
...
all_fcs$df %>% 
  transmute(filename, expr = map(fcs, exprs)) %>% 
  identity() ->
  x
walk2(x$filename, x$expr, function(x, y) {
